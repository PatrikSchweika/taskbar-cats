...
/** The window's HWND as a number. Electron hands it over as a little-endian buffer. */
function nativeHandle(window: BrowserWindow): number {
	const buffer = window.getNativeWindowHandle();
	return buffer.length >= 8
		? Number(buffer.readBigUInt64LE(0))
...
class CatsApp {
	...
	private _createOverlay(): void {
		...
		// So the tracker can ask which z-band we are in, relative to the taskbar.
		this._tracker.overlayHandle = nativeHandle(window);
	}
	...
	private _syncOverlay(overlay: Overlay): void {
		...
		const key = JSON.stringify([strip, desktop.floor, desktop.icons]);
